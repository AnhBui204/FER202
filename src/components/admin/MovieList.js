...
const MovieList = ({movies, onDelete, onEdit  }) => {
    return (
        <div className='container bg-white text-black space-y-3 my-5'>
            <h1 className='font-bold text-lg mt-3'>Manage Movie</h1>
            <div className='flex items-center'>
                <input type='text' className='mr-1 w-full rounded-md border border-gray-300 px-4 py-2' placeholder='Search movie' />
                ...
                <table className='min-w-full bg-white border border-gray-300 border-separate '>
                    <thead>
                        <tr className='border-b text-white'>
                            ...
                            <th className='px-6 py-4 text-center text-md font-medium bg-gray-100 text-gray-900 border border-gray-300'>Type</th>
                    ...
                    <tbody className='space-x-4 m-3'>
                        {movies.map((movie) => (
                            <tr key={movie.id}>
                                ...
                                <td className='text-sm text-center border border-gray-500'>{movie.duration}</td>
                                <td className='text-sm text-center border border-gray-500'>{movie.type}</td>
                                <td className='text-sm text-center border border-gray-500'>{movie.trailer}</td>
